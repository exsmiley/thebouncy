...
class Episode(object):
    ...
    def __init__(self, state_manager, opponent, train=True, epsilon=0.1):
...
class Opponent(object):
    ...
    def __init__(self, answer=None):
        self.game = Mastermind(target=answer)
...
class MastermindRL(object):
    ...
    def train(self):
        for epoch in tqdm.tqdm(xrange(self.num_epochs)):
            start = time.time()
    ...
    def _run_episode(self, answer, train=True, i=0, epoch=0):
        ...
        episode = self.episode(self.state_manager, opponent, train=train, epsilon=1-1.*(i+1)/self.num_epochs)
    ...
    def test(self):
        start = time.time()
        total_taken = 0.
        for i in tqdm.tqdm(xrange(NUM_OPTIONS**NUM_PEGS)):
            answer = get_guess(i)
            opponent = Opponent(answer=answer)
            episode = self.episode(self.state_manager, opponent, train=False, epsilon=0)
            actions_taken = map(lambda x: get_guess(x[1]), episode.run())
