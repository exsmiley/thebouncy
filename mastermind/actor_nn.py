...
class ActorModel(object):
    ...
    def create_graph(self):
        ...
        loss_policy = - log_prob * tf.stop_gradient(advantage)                                  # maximize policy (without value error)
        loss_value  = LOSS_V * tf.square(advantage)                                             # minimize value error
        entropy = LOSS_ENTROPY * tf.reduce_sum(self.actor_layer * tf.log(self.joined_out_layer + 1e-10), axis=1, keep_dims=True)   # maximize entropy (regularization)
    ...
    def train(self, states, actions, rewards):
        loss, minimize = self.session.run([self.loss_total, self.minimize], feed_dict={
...
class Environment(object):
    ...
    def run_episode(self, agent):
        ...
        while len(states) < 1000 and not won:
...
if __name__ == '__main__':
    ...
    # embedding =  encoder.get_embeddings([3, 4, 5, 4], (3, 0))

    # actor = ActorModel(chkpt=None)
    # print actor.get_choice_vectors(embedding)
    # print actor.get_action_vector(embedding)

    # print action_from_vector(actor.get_choice_vectors(embedding))
    TOTAL_GAMES = 1000000
